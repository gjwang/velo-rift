#!/bin/bash
# test_inception_no_manifest.sh - Test inception behavior when manifest is missing
# This should NOT hang - inception should fallback gracefully
...
echo "=== Test 1: Without inception ==="
./stat_test
echo ""

echo "=== Test 2: With inception (no manifest - should fallback, NOT hang) ==="
unset VRIFT_MANIFEST VRIFT_VFS_PREFIX VRIFT_PROJECT_ROOT

# Run with background and timeout check
DYLD_INSERT_LIBRARIES="$INCEPTION_PATH" VRIFT_DEBUG=1 ./stat_test 2>&1 &
PID=$!
