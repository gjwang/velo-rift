...
PROBE_BIN="$TEST_WORKSPACE/mutation_probe"
...
check_blocked "unlink"       "G-MUT.4"  "unlink() on VFS file blocked"
check_blocked "rename"       "G-MUT.5"  "rename() on VFS file blocked"
check_blocked "ftruncate"    "G-MUT.6"  "ftruncate() on VFS FD blocked"
check_blocked "fchmod"       "G-MUT.7"  "fchmod() on VFS FD blocked"
check_blocked "futimes"      "G-MUT.8"  "futimes() on VFS FD blocked"
check_blocked "unlinkat"     "G-MUT.9"  "unlinkat() on VFS file blocked"
check_blocked "renameat"     "G-MUT.10" "renameat() on VFS file blocked"
...
# Additional test: verify touch(1) specifically — the most common mtime bypass
log_test "G-MUT.15" "touch(1) command on VFS file blocked"
...
if [ "$TOUCH_EXIT" -ne 0 ]; then
    ...
    log_pass "touch(1) did not change mtime (may have been silently blocked)"
else
    log_fail "touch(1) CHANGED mtime ($ORIGINAL_MTIME -> $AFTER_MTIME) — MTIME BYPASS via utimes/setattrlist"
fi
