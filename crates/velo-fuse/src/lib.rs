...
mod imp {
    ...
    impl VeloFs {
        ...
        fn init_from_manifest(&mut self, manifest: &Manifest) {
            ...
            // Ensure root exists
            self.inodes.insert(1, InodeEntry {
            ...
            for path in paths {
                if path == "/" { continue; } // Already handled
                ...
                self.inodes.insert(inode, InodeEntry {
                ...
                if let Some(parent) = p.parent() {
                    let parent_str = if parent == Path::new("") { "/" } else { parent.to_str().unwrap() };
        ...
        fn vnode_to_attr(inode: u64, vnode: &VnodeEntry) -> FileAttr {
            FileAttr {
                ...
                kind: if vnode.is_dir() { FileType::Directory } else { FileType::RegularFile },
    ...
    impl Filesystem for VeloFs {
        ...
        fn readdir(
            ...
            if offset == 0 {
                ...
                if reply.add(1, 1, FileType::Directory, "..") { // Parent hardcoded to 1 for simplicity for now
                    return;
                }
            }
            ...
            for (i, (name, child_ino)) in entry.children.iter().enumerate().skip(skip) {
                let child_type = self.inodes.get(child_ino).map(|e| e.attr.kind).unwrap_or(FileType::RegularFile);
...
mod imp {
    ...
    /// Dummy FUSE filesystem for non-feature builds
    pub struct VeloFs;
