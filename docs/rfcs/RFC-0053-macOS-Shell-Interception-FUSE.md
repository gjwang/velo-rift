...
|----------|-----------|-------------|--------------|-----------|
| **FUSE-T** | ⭐⭐⭐ Complete | ~10-20% overhead | brew (admin) | ⭐⭐⭐ Best |
| **Wrapper Virtualization** | ⭐⭐ Partial | None | None | ⭐⭐ Bypassable |
| **Container** | ⭐⭐⭐ Complete | Heavy | Docker/Lima | ⭐⭐⭐ Best |
...
├── Cargo.toml
└── src/
    ├── lib.rs          // FUSE filesystem trait implementation
    ├── vnode_fs.rs     // VnodeEntry-based filesystem
    ├── cas_reader.rs   // Read blobs from CAS
    └── cow_writer.rs   // Copy-on-Write for modifications
```
...
> **Important**: Wrappers don't "block" operations—they **convert shell commands to virtualized daemon IPC calls**.
...
|----------|-------------------|--------|
| **Linux** | Namespace + OverlayFS | ✅ Implemented |
| **macOS** | Shim (DYLD_INSERT) | ⚠️ SIP-limited |
| **macOS** | FUSE-T | 🔜 To be implemented |
...
├─────────────────────────────────────────────────┤
│  OverlayFS (kernel)                             │
│    ├── lower layer: CAS blobs (read-only)       │
│    └── upper layer: /tmp/workspace (writable)   │
│  → Uses kernel OverlayFS directly, best perf    │
└─────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────┐
│                    macOS                        │
├─────────────────────────────────────────────────┤
│  No OverlayFS! Use FUSE-T to emulate:           │
│                                                 │
│  FUSE-T (userspace)                             │
│    └── vrift-fuse daemon                        │
│          ├── read → return blob from CAS        │
│          └── write → CoW to upper layer         │
│  → Use FUSE to implement OverlayFS semantics    │
└─────────────────────────────────────────────────┘
```
